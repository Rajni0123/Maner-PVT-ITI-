...
<main>
  ...
  <section class="relative h-[640px] flex items-center overflow-hidden">
    ...
    <div class="relative z-20 px-gutter max-w-container-max mx-auto w-full">
      <div class="max-w-2xl">
        <span class="hero-text-animate font-label-sm text-secondary-fixed-dim tracking-[0.2em] uppercase mb-4 block"><?= e($heroEyebrow) ?></span>
        <h1 class="hero-text-animate font-display text-display text-white mb-6">
          <?= $heroTitleHtml ?>
        </h1>
        <p class="hero-text-animate font-body-lg text-body-lg text-white/80 mb-10 leading-relaxed">
          <?= e($heroDesc) ?>
        </p>
        <div class="hero-text-animate flex flex-col sm:flex-row gap-4">
  ...
  <section class="py-section-gap px-gutter max-w-container-max mx-auto">
    ...
    <div class="grid md:grid-cols-2 gap-8">
      <?php $tradeIdx = 0; foreach ($trades as $t):
        $slug = $t['slug'] ?? '';
      ...
      ?>
      <div class="group border border-outline-variant overflow-hidden hover:shadow-xl transition-all duration-300" data-animate="fade-up" data-delay="<?= ($tradeIdx * 200) ?>"><?php $tradeIdx++; ?>
  ...
  <!-- Latest News & Updates + Eligibility (DESIGN.md tokens) -->
  <?php
  $notices = $notices ?? [];
  $defaultNewsItems = [
      'सत्र 2026-28 नामांकन जारी है........ सीट उपलब्ध रहने तक { सीमित सीटें }',
      'ITI के साथ-साथ इंटरमीडिएट (I.Sc) की डिग्री भी करें',
      'यहाँ विश्व स्तरीय ट्रेनिंग, प्रतिमाह जॉब प्लेसमेंट, निःशुल्क पाठ्यसामग्री एवं यूनिफॉर्म प्रदान किया जाता है',
      'ITI कोर्स करने के लिए सरकार दे रही है सहायता (BSCC / Scholarship)',
      'Maner Private ITI में Electrician एवं Fitter ट्रेड उपलब्ध',
      'NCVT के आदेशानुसार 2026-28 का Class : दिनांक 01 सितम्बर 2026 सुबह 09:00 बजे से प्रारंभ होगा',
      'प्रत्येक माह कैम्पस सेलेक्शन / प्लेसमेंट सहायता',
      'सरकार द्वारा निर्धारित कोर्स शुल्क — कुल प्रशिक्षण शुल्क Rs. 52,200/-',
  ];
  $newsItems = [];
  foreach (array_slice($notices, 0, 8) as $n) {
      $newsItems[] = [
          'text' => $n['title'] ?? '',
          'url' => !empty($n['pdf']) ? upload_url($n['pdf']) : site_url('notices'),
          'external' => !empty($n['pdf']),
      ];
  }
  if (empty($newsItems)) {
      foreach ($defaultNewsItems as $text) {
          $newsItems[] = ['text' => $text, 'url' => site_url('notices'), 'external' => false];
      }
  }
  $eligibilityItems = [
      '2026 में नामांकन 15 जनवरी से प्रारंभ',
      'निःशुल्क — कोर्स की किताब, बैग, डायरी, यूनिफॉर्म एवं आई डी कार्ड',
      '2026-28 का क्लास 01 सितम्बर से प्रारंभ',
      'नामांकन के लिए अपेक्षित योग्यता : विज्ञान के साथ 10वीं उत्तीर्ण या समकक्ष',
      'उम्र : 01 अगस्त को 14 से 30 वर्ष तक',
      'अनिवार्य वर्ग उपस्थिति : न्यूनतम 80%',
      'केवल नियमित ट्रेनिंग प्राप्त करने को इच्छुक छात्रों / छात्राओं के लिए ही नामांकन संभव है',
  ];
  ?>
  <section class="py-section-gap px-gutter max-w-container-max mx-auto" id="latest-news">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">

      <!-- Latest News & Updates -->
      <div class="bg-surface-container-lowest border border-outline-variant" data-animate="fade-up">
        <div class="bg-primary-container px-gutter py-6 flex items-center justify-between gap-4">
          <div>
            <span class="font-label-sm text-label-sm text-secondary-container uppercase tracking-widest block mb-1">Announcements</span>
            <h2 class="font-headline-md text-headline-md text-on-primary">Latest News &amp; Updates</h2>
          </div>
          <span class="material-symbols-outlined text-secondary-container text-4xl" style="font-variation-settings:'FILL' 1">campaign</span>
        </div>
        <ul class="divide-y divide-outline-variant">
          ...
          <li>
            <a href="<?= e($item['url']) ?>"
               ...
               class="flex items-start gap-3 px-gutter py-4 font-body-md text-body-md text-on-surface hover:bg-surface-container-low transition-colors group">
              <span class="material-symbols-outlined text-secondary-container text-[20px] mt-0.5 shrink-0">arrow_right</span>
              <span class="group-hover:text-primary transition-colors"><?= e($item['text']) ?></span>
            </a>
          </li>
          <?php endforeach; ?>
        </ul>
        <div class="px-gutter py-5 border-t border-outline-variant flex flex-wrap gap-4">
          <a href="<?= site_url('notices') ?>" class="inline-flex items-center gap-2 font-bold text-primary hover:underline">
            View More Updates
            <span class="material-symbols-outlined text-[18px]">arrow_forward</span>
          </a>
          <a href="<?= site_url('apply-admission') ?>" class="inline-flex items-center gap-2 font-bold text-secondary hover:underline">
            Apply Now
      ...
      <div class="bg-surface-container-lowest border border-outline-variant" data-animate="fade-up" data-delay="200">
        <div class="bg-secondary-container px-gutter py-6 flex items-center justify-between gap-4">
          <div>
            <span class="font-label-sm text-label-sm text-on-secondary-container uppercase tracking-widest block mb-1">Admission 2026–28</span>
            <h2 class="font-headline-md text-headline-md text-on-secondary-container">Eligibility for Admission</h2>
          </div>
          <span class="material-symbols-outlined text-on-secondary-container text-4xl" style="font-variation-settings:'FILL' 1">school</span>
        </div>
        <ul class="divide-y divide-outline-variant">
          <?php foreach ($eligibilityItems as $item): ?>
          <li class="flex items-start gap-3 px-gutter py-4 font-body-md text-body-md text-on-surface">
        ...
        <div class="px-gutter py-5 border-t border-outline-variant flex flex-wrap gap-4">
          <a href="<?= site_url('fee-structure') ?>" class="inline-flex items-center gap-2 bg-primary text-on-primary px-5 py-2.5 font-bold hover:opacity-90 transition-opacity">
            View Fee Structure
          </a>
          <a href="<?= site_url('admission-process') ?>" class="inline-flex items-center gap-2 border border-primary text-primary px-5 py-2.5 font-bold hover:bg-primary hover:text-on-primary transition-colors">
            Admission Process
          </a>
          <a href="<?= site_url('apply-admission') ?>" class="inline-flex items-center gap-2 font-bold text-secondary hover:underline">
            Online Registration
  ...
  <section class="py-section-gap px-gutter max-w-container-max mx-auto">
    ...
    <div class="grid md:grid-cols-3 gap-0 border border-outline-variant">
      <div class="p-12 border-b md:border-b-0 md:border-r border-outline-variant hover:bg-surface-container-low transition-colors group" data-animate="fade-up" data-delay="100">
      ...
      <div class="p-12 border-b md:border-b-0 md:border-r border-outline-variant hover:bg-surface-container-low transition-colors group" data-animate="fade-up" data-delay="300">
        <div class="w-16 h-16 bg-primary text-white flex items-center justify-center mb-8 group-hover:rotate-12 transition-transform">
      ...
      <div class="p-12 hover:bg-surface-container-low transition-colors group" data-animate="fade-up" data-delay="500">
        <div class="w-16 h-16 bg-primary text-white flex items-center justify-center mb-8 group-hover:rotate-12 transition-transform">
  ...
  <!-- CTA Section -->
  <section class="bg-surface-container-highest border-t border-outline-variant py-20 px-gutter" data-animate="zoom-in">
...
<div class="floating-cta" id="floatingCTA">
  <a href="<?= site_url('apply-admission') ?>" class="fab-btn fab-btn-admission" title="Apply Now">
    <span class="fab-tooltip">Apply Now</span>
    <span class="material-symbols-outlined" style="font-variation-settings:'FILL' 1">school</span>
  </a>
  <a href="https://wa.me/<?= e($whatsappPhone) ?>?text=<?= e($whatsappMsg) ?>" target="_blank" rel="noopener" class="fab-btn fab-btn-whatsapp" title="WhatsApp">
  ...
  <a href="tel:+<?= e($callPhone) ?>" class="fab-btn fab-btn-call" title="Call Now">
    <span class="fab-pulse"></span>
