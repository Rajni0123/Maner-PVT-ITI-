...
class SiteData
{
    ...
    public static function menus(): array
    {
        ...
        $rows = Database::fetchAll(
            'SELECT * FROM menus WHERE is_active = 1 ORDER BY order_index, id'
        );

        return self::buildMenuTree($rows);
    }

    public static function menusFlat(): array
    {
        ensure_important_links_menu();
    ...
    private static function buildMenuTree(array $rows): array
    {
        $byParent = [];
        foreach ($rows as $row) {
